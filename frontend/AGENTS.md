...
The AfterFlow approval desk lives at `/workspace/after-sales`. Its API/types are in `src/core/after-sales.ts`; only admins can list or mutate Action Requests. The UI always submits the displayed action version and approved payload, while the Gateway remains authoritative for role, optimistic-lock, hash, expiry, balance, and idempotency checks.
