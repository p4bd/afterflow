...
#### AfterFlow vertical-agent example

This repository includes a runnable ecommerce after-sales example. It combines 12 business tools, seven workflow skills, deterministic refund and reverse-fulfillment engines, denominator-aware operations alerts, and 30 Gold Case scenarios. Refund amounts use integer minor currency units and are never calculated by the LLM. High-risk or over-limit refunds become persisted Action Requests; authenticated admins approve them in `/workspace/after-sales`, then execution rechecks the payload hash, version, expiry, balance, and idempotency key against a mock payment adapter.

- Business code: `backend/app/after_sales/`
- Skills: `skills/public/after-sales-*`
- Custom-agent template: `docs/examples/afterflow-agent/`
- Tool registration: the `after-sales` group in `config.example.yaml`
- Approval desk: `/workspace/after-sales`
- Evaluation: `backend/tests/fixtures/after_sales_evaluation_100.json`

Copy the AfterFlow tool group, tools, and fail-closed Guardrail example into an existing `config.yaml` (or regenerate it from the example), then create a Custom Agent with the template's tool-group and skill whitelist. Local demo orders are `ORDER-1001` through `ORDER-1003`; the payment executor is intentionally local and must be replaced by a provider adapter before production use.

To compare an external baseline, save its predictions as a JSON object keyed by evaluation case ID, then run `cd backend && uv run python scripts/evaluate_after_sales_predictions.py predictions.json`. No baseline score is fabricated when a model has not actually been run.
