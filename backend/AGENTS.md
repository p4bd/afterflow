...
│   │   ├── after_sales/       # AfterFlow domain contracts, deterministic rules, mock adapters, and LangChain tools
...
### AfterFlow vertical domain (`app/after_sales/`)

AfterFlow is the committed ecommerce after-sales example. `schemas.py` owns its stable business contracts, `decision.py` handles refund eligibility, `reverse.py` compares return/replacement economics, `risk_ops.py` detects denominator-aware operating anomalies, `mock_data.py` is replaceable demo data, and `tools.py` is the LangChain adapter loaded through `config.yaml`. Keep money in integer minor units. The LLM may orchestrate and explain a decision but must not calculate amounts or bypass approval signals. Production integrations should replace data adapters while preserving the decision contract; do not move product-specific imports into `deerflow-harness`.

Phase 2 persistence uses the registered ORM rows in `deerflow.persistence.after_sales`, migration `0004_after_sales`, and the app-layer repository in `app.after_sales.repository`. Gateway routes live at `/api/after-sales`; authenticated admins are the demo supervisor/executor role. `AfterSalesGuardrailProvider` protects Agent write tools, while `actions.py` repeats every authorization-critical state, hash, version, expiry, balance, and idempotency check so direct API calls cannot bypass policy.

Tests live in `tests/test_after_sales_*.py`; Gold Case fixtures live in `tests/fixtures/after_sales_gold_cases.json`.
The fixed cross-domain evaluation set is `tests/fixtures/after_sales_evaluation_100.json`; score real external-model predictions with `uv run python scripts/evaluate_after_sales_predictions.py <predictions.json>`.
